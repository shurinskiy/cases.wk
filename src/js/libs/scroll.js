...
export const scrollBasedToggle = (sticky, items, options = {}) => {
	...
	if (items.length) {
		const classToggle = (items, wrapper, active) => {
			const box = wrapper.getBoundingClientRect();
			const step = Math.floor(Math.abs(box.top) / wrapper.scrollHeight * (items.length + 1));
...
* far - расстояние до самого дальнего слайда (от плоскости экрана)
...
export const deepParallax = (container, items, options = {}) => {
	...
	let initDeep = () => {
		...
		items.forEach((item, i) => {
			...
			if (top <= 0) {
				item.style.transform = `translateZ(${far * i}px)`;
				
			} else if (bottom >= _wrapper.scrollHeight) {
				item.style.transform = `translateZ(${(far * i) + max}px)`;
	...
	let moveDeep = () => {
		...
		if (top > 0 && bottom < _wrapper.scrollHeight) {
			...
			items.forEach((item, i) => {
				...
				item.style.opacity = move < Math.abs(far) / fade ? 1 : 0;
			});
		}
	};
